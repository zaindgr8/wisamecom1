...
const _removepathprefix = __webpack_require__(23051);
const _pathhasprefix = __webpack_require__(31342);
...
function validateCartItems(inventorySrc, cartDetails) {
    ...
    for(const id in cartDetails){
        ...
        if (typeof inventoryItem.description === "string" && inventoryItem.description.length > 0) item.price_data.product_data.description = inventoryItem.description;
        if (typeof inventoryItem.image === "string" && inventoryItem.image.length > 0) item.price_data.product_data.images = [
            inventoryItem.image
        ];
...
function formatLineItems(cartDetails) {
    const lineItems = [];
    for(const id in cartDetails)lineItems.push({
        price: id,
        quantity: cartDetails[id].quantity
    });
    return lineItems;
